...
import { BFS, DFS, dijkstra, generateRandomMaze } from "~/app";
import { AlgorithmOption, CellInterface, SearchingAlgoEnum } from "~/interfaces";
...
const GridBoard = () => {
  ...
  const visualizeAlgo = (type: SearchingAlgoEnum) => {
    ...
    const path = getShortestPathCells(end);
  ...
  return (
    <>
      <Modal open={hasPathError} onClose={() => setHasPathError(false)} />
      ...
      <div className="bg-gray-900 pt-4">
        <div className="mx-auto flex max-w-[80%] md:flex-row flex-col items-center justify-between">
          <div className="flex flex-1 flex-wrap md:flex-row flex-col gap-4 items-start md:items-center w-full justify-start space-x-4 mx-4">
            ...
            >
              <MapPinIcon className="h-5 w-5 mr-2" /> Limpar Caminho
            </button>
            ...
            <button
              ...
              className="items-center w-fit disabled:bg-green-500 disabled:cursor-not-allowed inline-flex bg-green-600 text-[15px] text-white px-4 py-2 rounded-md"
            >
              <TrashIcon className="h-5 w-5 mr-2" /> Limpar Tudo
            </button>
            ...
            >
              <ForwardIcon className="h-5 w-5 mr-2" /> Velocidade: {speed}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="w-full bg-gray-900">
        <div className="flex md:gap-0 flex-wrap gap-4 flex-1 pt-4 max-w-[80%] md:flex-row flex-col items-start md:items-center justify-start space-x-4 mx-auto">
          <button
          ...
          >
            <GithubLogo /> Código Fonte
          </a>
        </div>
      </div>

      <div className="w-full bg-gray-900 ">
        <div className="flex gap-6 justify-center max-w-[80%] mx-auto items-center">
          <StatsSection
            stats={[
              {
                name: "Células escaneadas",
                icon: MagnifyingGlassIcon,
                data: cellsScanned.toString(),
              },
              {
                name: "Células percorridas",
      ...
      {selectedAlgo ? (
        <div className="flex my-3 w-full mx-auto justify-end max-w-[80%]">
          <button
